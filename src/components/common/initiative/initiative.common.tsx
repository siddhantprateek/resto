...
const Initiative = (props: IInitiative) => {
  ...
  return (
    <div className={`initiative-card 
      ${ props.name === "Waste Reduction" ? "display-flex" : "display-flex-reverse"}
    `} ref={initRef}>
      <div className={`left-init-content ${initView ? "trans-from-right-less": ""}`}>
      <p className='purple'>&#91; Initiative &#93; </p>
      <h1 className='init-title'>{props.name}</h1>
      <p className='init-dp'>{props.description}</p>
      </div>
      <div className={`right-init-content ${initView ? "trans-from-left": ""}`}>
