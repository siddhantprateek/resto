...
const Testimony = (props: IReviews) => {
  ...
  const { ref: testimonyRef, inView: testimonyView } = useInView({
    threshold: 0,
  });
  return (
    <div
      className={`testinomy ${testimonyView ? "trans-from-buttom" : ""}`}
      ref={testimonyRef}
    >
      ...
      <div className="review">
        <div className="rating">
          <p>
            <Rating readonly={true} initialValue={props.rating} />
          </p>
        </div>
        <p className="testinomy-content">{props.comment}</p>
      </div>
    </div>
  );
};
